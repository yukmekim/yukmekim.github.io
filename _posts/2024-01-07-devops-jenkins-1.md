...
도서에서는 윈도우 환경에서 jenkins를 설치해서 진행하는 방법을 소개하고 있어 젠킨스 내부 시스템에 대한 참고로 사용하고, 팀장님께 건의 드릴 ec2 서버를 이용해 보기로 했다. aws는 무려 **750시간이라는 프리티어 서비스**를 제공하는데 한 번 이용해보려 한다.
...
이전에 SSEN(직장동료)이 프리티어로 젠킨스 구축을 시도해봤는데 서버도 느리고 젠킨스 파이프라인이 실행되지 않는 문제가 있었다고 알려줬다.

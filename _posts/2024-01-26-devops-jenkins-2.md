...
`enviroment`에 파이프라인 동작에 사용할 환경 변수를 선언해두었다. 저장소 url이나 pom파일 위치등은 작업 단위에 작성하는것 보다는 변수를 만들어 사용하는게 편하다.  
...
해당 파이프라인을 실행했을때 `/var/lib/jenkins/workspace/젠킨스 프로젝트 명/` 디렉토리에 패키징 되지 않은 소스를 받을 수 있었다.  
...
작성한 파이프라인이 정상 작동하는지 확인하기 위해 실행 시켜 봤는데, 프리티어에서 계속해서 테스트를 진행할수 없는 상태가 되었다.  

<img src="/assets/images/post/jenkins_2/jenkins_node_error.png" width="80%" height="80%"/>
(대충 노드가 실행되기 위해서 1GiB의 메모리가 필요한데 417MiB 밖에 없다는 내용)

node agent를 통해서 스케쥴링 및 빌드를 해준다고 하는데, 메모리 부족으로인해 node가 오프라인 상태로 유지가 되어 파이프라인이 빌드되지 않는 상태이다.

ec2 서버의 메모리를 올려서 계속해서 진행 또는 맥북에 젠킨스를 설치하고 sshPublisher 플러그인을 사용하여 ec2 서버에 소스를 배포하는 방향으로 진행하겠다.